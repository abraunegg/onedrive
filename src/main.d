	...
	if ((cfg.getValueBool("synchronize")) || (cfg.getValueBool("resync")) || (cfg.getValueBool("monitor"))) {
		...
		if ((cfg.getValueBool("synchronize")) || (cfg.getValueBool("resync"))) {
			if (online) {
				...
				performSync(sync, cfg.getValueString("single_directory"), cfg.getValueBool("download_only"), cfg.getValueBool("local_first"), cfg.getValueBool("upload_only"), LOG_NORMAL, true, syncListConfigured, cfg.getValueString("skip_dir"));
			}
		}
				...
				if (currTime - lastCheckTime > checkInterval) {
					...
					try {
						...
						try {
							...
							performSync(sync, cfg.getValueString("single_directory"), cfg.getValueBool("download_only"), cfg.getValueBool("local_first"), cfg.getValueBool("upload_only"), (logMonitorCounter == logInterval ? MONITOR_LOG_QUIET : MONITOR_LOG_SILENT), fullScanRequired, syncListConfiguredOverride, cfg.getValueString("skip_dir"));
							if (!cfg.getValueBool("download_only")) {
...
void performSync(SyncEngine sync, string singleDirectory, bool downloadOnly, bool localFirst, bool uploadOnly, long logLevel, bool fullScanRequired, bool syncListConfigured, string skipDirValue)
{
	int count;
	string remotePath = "/";
    string localPath = ".";
						...
						if (fullScanRequired) {
							...
							if (!downloadOnly) {
								...
								if (syncListConfigured) {
									...
									// Perform the sync based on fullScanRequired being set, but only IF skip_dir is set
									if (skipDirValue != "") {
										// Perform a full scan based on fullScanRequired being set, as potential for changes to have been discarded
										sync.applyDifferences(fullScanRequired);
									} else {
										// skip_dir was empty, so no changes would have been discarded
										sync.applyDifferences(false);
									}
								}
							}
