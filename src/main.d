		...
		if (cfg.getValueBool("monitor")) {
			...
			m.onDirCreated = delegate(string path) {
				...
				if ((cfg.getValueBool("skip_dotfiles")) && (selectiveSync.isDotFile(path))) {
					log.vlog("[--monitor] Skipping watching local path - .folder found & --skip-dot-files enabled: ", path);
				} else {
					log.vlog("[--monitor] Local directory created: ", path);
					try {
			...
			m.onFileChanged = delegate(string path) {
				log.vlog("[--monitor] Local file changed: ", path);
				try {
					sync.scanForDifferences(path);
				} catch (CurlException e) {
			...
			m.onDelete = delegate(string path) {
				...
				log.vlog("[--monitor] Local item deleted: ", path);
				try {
			...
			m.onMove = delegate(string from, string to) {
				log.vlog("[--monitor] Local item moved: ", from, " -> ", to);
				try {
