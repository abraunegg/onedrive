	...
	if ((synchronize) || (resync) || (monitor)) {
		...
		if ((synchronize) || (resync)) {
			if (online) {
				...
				// Perform the sync
				performSync(sync, singleDirectory, downloadOnly, localFirst, uploadOnly, LOG_NORMAL, monitor);
			}
		}
			...
			while (true) {
				...
				if (currTime - lastCheckTime > checkInterval) {
					...
					try {
						...
						try {
							performSync(sync, singleDirectory, downloadOnly, localFirst, uploadOnly, (logMonitorCounter == logInterval ? MONITOR_LOG_QUIET : MONITOR_LOG_SILENT), monitor);
							if (!downloadOnly) {
...
void performSync(SyncEngine sync, string singleDirectory, bool downloadOnly, bool localFirst, bool uploadOnly, long logLevel, bool monitor)
{
	int count;
	string remotePath = "/";
    string localPath = ".";
