	...
	if (appConfig.syncEngineWasInitialised) {
		...
		if (appConfig.getValueBool("monitor")) {
			...
			monitorLoop: while (performFileSystemMonitoring) {
				...
				// Process any already queued local filesystem monitor events at the start of the loop. 
				// This prevents a WebSocket/Webhook notification with an empty /delta response from overtaking and cancelling
				//  a genuine local delete or modify event.
				if (!appConfig.getValueBool("download_only")) {
				...
				if ((currentTime - lastCheckTime >= checkOnlineInterval) || (monitorLoopFullCount == 0)) {
					...
					if (testInternetReachability(appConfig)) {
						...
						// Handle any local filesystem monitor events that arrived while the scheduled synchronisation pass was running. 
						// Do not leave these queued until the next monitor interval.
						if (!appConfig.getValueBool("download_only")) {
							processInotifyEvents(true);
						}
				...
				if (performFileSystemMonitoring) {	
					...
					if (filesystemMonitor.initialised || webhookEnabled || oneDriveSocketIo !is null) {
						...
						if (res > 0) {
							...
							// Do not drain queued local worker wake messages here.
							// A wake queued while processInotifyEvents(true) was running may represent
							// genuine new local filesystem work, not stale work. Leave it in the
							// mailbox so the next monitor iteration can process it normally.
...
void oneDriveOnlineCallback() {
	// WebSocket/Webhook notifications can arrive at the same time as genuine local filesystem activity. 
	// Consume any queued local inotify work before running online reconciliation so an empty /delta response cannot cancel a real local delete or modify.
	if (!appConfig.getValueBool("download_only")) {
		processInotifyEvents(true);
	}
	...
	if (!appConfig.getValueBool("upload_only")) {
		...
		// Online reconciliation can create, remove, or update local filesystem
		// paths. Any inotify events generated by applying that remote state are
		// reconciliation side-effects, not user initiated local work, and must
		// not be replayed as uploads after the callback completes.
		if (!appConfig.getValueBool("download_only")) {
			if (debugLogging) {addLogEntry("Draining local filesystem monitor events generated by online callback reconciliation while preserving local delete events", ["debug"]);}
			drainLocalMonitorEventsPreservingDeletes();
		}
	}
}
...
void performStandardSyncProcess(string localPath, Monitor filesystemMonitor = null) {
	...
	if (appConfig.getValueBool("local_first")) {
		...
		syncEngineInstance.syncOneDriveAccountToLocalDisk();
		// Drain any local monitor events generated by applying remote state, but preserve delete callbacks.
		// Create/change notifications are reconciliation side-effects; delete notifications may be genuine local deletes that arrived while online reconciliation was running.
		drainLocalMonitorEventsPreservingDeletes();
...
// This is used during --download-only monitor loops, where local filesystem
// notifications must not be replayed as user initiated upload work.
...
// Drain queued local filesystem monitor events generated during online reconciliation,
// but preserve delete callbacks. Remote reconciliation side-effects can safely discard
// create/change notifications, however real local deletes may arrive while a sync pass
// is running. Dropping those delete events leaves remote parent directories behind and
// forces the later database consistency scan to delete only child files.
void drainLocalMonitorEventsPreservingDeletes() {
	if ((filesystemMonitor is null) || (!filesystemMonitor.initialised)) {
		return;
	}
...
void processInotifyEvents(bool updateFlag, bool processDeletesWhenDraining = false) {
	...
	try {
		...
		// True = process all callbacks
		// False = cancel non-delete callbacks; processDeletesWhenDraining preserves delete callbacks when requested
