	...
	void loadSyncList(string filepath) {
		...
		foreach (line; range) {
			...
			if (cleanLine == "/*" || cleanLine == "/") {
				string errorMessage = "ERROR: Invalid sync_list rule '" ~ to!string(cleanLine) ~ "' detected. Please use 'sync_root_files = \"true\"' or --sync-root-files option to sync files in the root path. To sync directories in the root path, include them manually, as `/*` is not permitted.";
				addLogEntry();
				addLogEntry(errorMessage, ["info", "notify"]);
				addLogEntry();
				// do not add this rule
