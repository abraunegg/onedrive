...
class ClientSideFiltering {
	...
	bool initialise() {
		...
		logBuffer.addLogEntry("Configuring Client Side Filtering (Selective Sync)", ["debug"]);
		...
		logBuffer.addLogEntry("Configuring skip_dir ...", ["debug"]);
		logBuffer.addLogEntry("skip_dir: " ~ to!string(appConfig.getValueString("skip_dir")), ["debug"]);
		setDirMask(appConfig.getValueString("skip_dir"));
		
		// Was --skip-dir-strict-match configured?
		logBuffer.addLogEntry("Configuring skip_dir_strict_match ...", ["debug"]);
		logBuffer.addLogEntry("skip_dir_strict_match: " ~ to!string(appConfig.getValueBool("skip_dir_strict_match")), ["debug"]);
		if (appConfig.getValueBool("skip_dir_strict_match")) {
			setSkipDirStrictMatch();
		}
		
		// Was --skip-dot-files configured?
		logBuffer.addLogEntry("Configuring skip_dotfiles ...", ["debug"]);
		logBuffer.addLogEntry("skip_dotfiles: " ~ to!string(appConfig.getValueBool("skip_dotfiles")), ["debug"]);
		if (appConfig.getValueBool("skip_dotfiles")) {
			setSkipDotfiles();
		}
		
		// Handle skip_file configuration in config file
		logBuffer.addLogEntry("Configuring skip_file ...", ["debug"]);
		...
		foreach(entry; appConfig.getValueString("skip_file").split("|")){
			if (entry == ".*") {
				...
				logBuffer.addLogEntry("ERROR: Invalid skip_file entry '.*' detected");
				return false;
			}
		}
		
		// All skip_file entries are valid
		logBuffer.addLogEntry("skip_file: " ~ appConfig.getValueString("skip_file"), ["debug"]);
	...
	void setFileMask(const(char)[] mask) {
		...
		logBuffer.addLogEntry("Selective Sync File Mask: " ~ to!string(fileMask), ["debug"]);
	}

	// Configure the regex that will be used for 'skip_dir'
	void setDirMask(const(char)[] dirmask) {
		directoryMask = wild2regex(dirmask);
		logBuffer.addLogEntry("Selective Sync Directory Mask: " ~ to!string(directoryMask), ["debug"]);
	}
	...
	bool isDirNameExcluded(string name) {
		...
		logBuffer.addLogEntry("skip_dir evaluation for: " ~ name, ["debug"]);
		
		// Try full path match first
		if (!name.matchFirst(directoryMask).empty) {
			logBuffer.addLogEntry("'!name.matchFirst(directoryMask).empty' returned true = matched", ["debug"]);
			return true;
		} else {
			// Do we check the base name as well?
			if (!skipDirStrictMatch) {
				logBuffer.addLogEntry("No Strict Matching Enforced", ["debug"]);
				...
				foreach_reverse(directory; pathSplitter(path)) {
					if (directory != "/") {
						...
						if(!checkPath.strip('/').matchFirst(directoryMask).empty) {
							logBuffer.addLogEntry("'!checkPath.matchFirst(directoryMask).empty' returned true = matched", ["debug"]);
							return true;
						}
					}
				}
				...
				// No match
				logBuffer.addLogEntry("Strict Matching Enforced - No Match", ["debug"]);
			}
		}
		// no match
		return false;
	...
	bool isFileNameExcluded(string name) {
		...
		// Returns false if no match
		logBuffer.addLogEntry("skip_file evaluation for: " ~ name, ["debug"]);
	...
	private bool isPathExcluded(string path, string[] allowedPaths) {
		...
		logBuffer.addLogEntry("Evaluation against 'sync_list' for this path: " ~ path, ["debug"]);
		logBuffer.addLogEntry("[S]exclude           = " ~  to!string(exclude), ["debug"]);
		logBuffer.addLogEntry("[S]exludeDirectMatch = " ~ to!string(exludeDirectMatch), ["debug"]);
		logBuffer.addLogEntry("[S]excludeMatched    = " ~ to!string(excludeMatched), ["debug"]);
		...
		foreach (allowedPath; allowedPaths) {
			...
			logBuffer.addLogEntry("Evaluation against 'sync_list' entry: " ~ allowedPath, ["debug"]);
			...
			if (comm.length == path.length) {
				...
				if (path == strippedAllowedPath) {
					...
					logBuffer.addLogEntry("Exact path match with 'sync_list' entry", ["debug"]);
					
					if (!exclude) {
						logBuffer.addLogEntry("Evaluation against 'sync_list' result: direct match", ["debug"]);
						finalResult = false;
						// direct match, break and go sync
						break;
					} else {
						logBuffer.addLogEntry("Evaluation against 'sync_list' result: direct match - path to be excluded", ["debug"]);
					...
					logBuffer.addLogEntry("Something 'common' matches the 'sync_list' input path", ["debug"]);
					...
					foreach(base; splitAllowedPaths) {
						...
						if (path == pathToEvaluate) {
							...
							if (!exclude) {
								logBuffer.addLogEntry("Evaluation against 'sync_list' result: direct match for parental path item", ["debug"]);
								finalResult = false;
								// direct match, break and go sync
								break;
							} else {
								logBuffer.addLogEntry("Evaluation against 'sync_list' result: direct match for parental path item but to be excluded", ["debug"]);
								finalResult = true;
								// do not set excludeMatched = true here, otherwise parental path also gets excluded
							}					
						}
			...
			if (comm.length == allowedPath[offset..$].length) {
				...
				if (canFind(path, subItemPathCheck)) {
					...
					if (!exclude) {
						logBuffer.addLogEntry("Evaluation against 'sync_list' result: parental path match", ["debug"]);
						finalResult = false;
						// parental path matches, break and go sync
						break;
					} else {
						logBuffer.addLogEntry("Evaluation against 'sync_list' result: parental path match but must be excluded", ["debug"]);
						finalResult = true;
						excludeMatched = true;
					}
				}
			...
			if (canFind(allowedPath[offset..$], wildcard)) {
				...
				if (matchAll(path, allowedMask)) {
					...
					if (!exclude && !excludeMatched && !exludeDirectMatch) {
						...
						logBuffer.addLogEntry("Evaluation against 'sync_list' result: wildcard pattern match", ["debug"]);
						finalResult = false;
					} else {
						logBuffer.addLogEntry("Evaluation against 'sync_list' result: wildcard pattern matched but must be excluded", ["debug"]);
						finalResult = true;
						excludeMatched = true;
					}
				}
		...
		// Interim results
		logBuffer.addLogEntry("[F]exclude           = " ~ to!string(exclude), ["debug"]);
		logBuffer.addLogEntry("[F]exludeDirectMatch = " ~ to!string(exludeDirectMatch), ["debug"]);
		logBuffer.addLogEntry("[F]excludeMatched    = " ~ to!string(excludeMatched), ["debug"]);
		...
		// results
		if (finalResult) {
			logBuffer.addLogEntry("Evaluation against 'sync_list' final result: EXCLUDED", ["debug"]);
		} else {
			logBuffer.addLogEntry("Evaluation against 'sync_list' final result: included for sync", ["debug"]);
		}
		return finalResult;
	}
}