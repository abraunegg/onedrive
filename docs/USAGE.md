...
### curl compatibility
If your system utilises curl >= 7.62.0 and < 7.47.0 curl defaults to prefer HTTP/2 over HTTP/1.1 by default. 

If your system utilises curl >= 7.47.0 will prefer HTTP/2 for HTTPS but stick to HTTP/1.1 by default.

If you wish to explicitly use HTTP/1.1 you will need to use the `--force-http-11` flag or set the config option `force_http_11 = "true"` to enable the application to use HTTP/1.1 otherwise all operations will use whatever is the curl default for your distribution.
