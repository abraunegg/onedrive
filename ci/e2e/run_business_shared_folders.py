...
from testcases_business_shared_folders.bsftc0001_clean_sync_pull_down import (
...
def _normalise_case_id(value: str) -> str:
    ...
    if not text:
        return ""
    if text.startswith("bsftc"):
        suffix = text[5:]
        if suffix.isdigit() and len(suffix) <= 4:
            return f"bsftc{suffix.zfill(4)}"
        return text
    if text.startswith("bsf"):
        suffix = text[3:]
        if suffix.isdigit() and len(suffix) <= 4:
            return f"bsftc{suffix.zfill(4)}"
        return text
    if text.isdigit() and len(text) <= 4:
        return f"bsftc{text.zfill(4)}"
    return text
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run OneDrive Business Account Shared Folder E2E test cases")
    parser.add_argument("--case-id", action="append", help="Run only the specified Business shared-folder case id(s). Supports bsftc0001, bsf0001, or 0001.")
    ...
    if context.e2e_target != "business-shared-folders":
        raise RuntimeError(
            "run_business_shared_folders.py must be executed with E2E_TARGET=business-shared-folders"
        )
    ...
    context.log(
        f"Initialising Business Shared Folder E2E framework for target='{context.e2e_target}', "
        f"run_id='{context.run_id}', run_label='{context.run_label}', debug_enabled={context.debug_enabled}"
    )
    context.log("Suite-wide cleanup is intentionally disabled for Business Shared Folder testing")
