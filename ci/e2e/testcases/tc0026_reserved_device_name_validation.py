...
class TestCase0026ReservedDeviceNameValidation(E2ETestCase):
    case_id = "0026"
    name = "reserved device name validation"
    description = "Validate reserved Windows device names are blocked while valid lookalike names still synchronise"
    ...
    def run(self, context: E2EContext) -> TestResult:
        ...
        root_name = f"ZZ_E2E_TC0026_{context.run_id}_{os.getpid()}"

        valid_files = [
            f"{root_name}/HelloCOM2.rar",
            f"{root_name}/Report_CON_v2.txt",
            f"{root_name}/LPT_notes.txt",
            f"{root_name}/valid_subdir/nested_valid_file.dat",
        ]

        invalid_files = [
            f"{root_name}/CON",
            f"{root_name}/CON.text",
            f"{root_name}/PRN",
            f"{root_name}/AUX",
            f"{root_name}/NUL",
            f"{root_name}/COM1",
        ...
        expected_skip_markers = [
            'Skipping item - invalid name (Microsoft Naming Convention): ./' + item
