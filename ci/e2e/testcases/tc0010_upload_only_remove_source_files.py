...
class TestCase0010UploadOnlyRemoveSourceFiles(E2ETestCase):
    ...
    def run(self, context: E2EContext) -> TestResult:
        ...
        command = [context.onedrive_bin, "--display-running-config", "--sync", "--verbose", "--upload-only", "--remove-source-files", "--resync", "--resync-auth", "--single-directory", root_name, "--syncdir", str(sync_root), "--confdir", str(upload_conf)]
        ...
        verify_command = [context.onedrive_bin, "--display-running-config", "--sync", "--verbose", "--download-only", "--resync", "--resync-auth", "--single-directory", root_name, "--syncdir", str(verify_root), "--confdir", str(verify_conf)]
        ...
        write_text_file(metadata_file, "\n".join([f"root_name={root_name}", f"returncode={result.returncode}", f"verify_returncode={verify_result.returncode}", f"verify_scope={root_name}"]) + "\n")
        ...
        details = {"returncode": result.returncode, "verify_returncode": verify_result.returncode, "root_name": root_name, "verify_scope": root_name}
        if result.returncode != 0: return self.fail_result(self.case_id, self.name, f"--upload-only with remove_source_files failed with status {result.returncode}", artifacts, details)
        ...
        if source_file.exists() or f"{root_name}/source.txt" in post_manifest: return self.fail_result(self.case_id, self.name, "Local source file still exists after remove_source_files processing", artifacts, details)
        if f"{root_name}/source.txt" not in remote_manifest: return self.fail_result(self.case_id, self.name, f"Remote file missing after upload-only remove_source_files: {root_name}/source.txt", artifacts, details)
