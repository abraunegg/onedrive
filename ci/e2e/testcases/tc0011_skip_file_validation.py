...
class TestCase0011SkipFileValidation(E2ETestCase):
    ...
    def run(self, context: E2EContext) -> TestResult:
        ...
        command = [context.onedrive_bin, "--display-running-config", "--sync", "--verbose", "--resync", "--resync-auth", "--single-directory", root_name, "--syncdir", str(sync_root), "--confdir", str(confdir)]
        ...
        if result.returncode != 0: return self.fail_result(self.case_id, self.name, f"skip_file validation failed with status {result.returncode}", artifacts, details)
        ...
        if f"{root_name}/keep.txt" not in remote_manifest: return self.fail_result(self.case_id, self.name, f"Expected non-skipped file missing remotely: {root_name}/keep.txt", artifacts, details)
        for unwanted in [f"{root_name}/skip.tmp", f"{root_name}/editor.swp"]:
            if unwanted in remote_manifest: return self.fail_result(self.case_id, self.name, f"skip_file pattern failed, file was synchronised: {unwanted}", artifacts, details)
