...
class TestCase0051MonitorModeMtimeOnlyLocalChangeHandling(MonitorModeTestCaseBase):
    ...
    description = "Touch an existing local file under --monitor without changing content and validate the mtime propagates"
    ...
    def run(self, context: E2EContext) -> TestResult:
        ...
        initial_content = (
            ...
            "This file content must remain unchanged; only the mtime is updated.\n"
        )

        extra_config_lines = ['force_session_upload = "true"']
        context.prepare_minimal_config_dir(conf_main, self._build_config_text(sync_root, app_log_dir, extra_config_lines=extra_config_lines))
        context.prepare_minimal_config_dir(conf_verify_initial, ("# tc0051 verify initial\n" f'sync_dir = "{verify_initial_root}"\n' 'bypass_data_preservation = "true"\n'))
        context.prepare_minimal_config_dir(conf_verify_final, ("# tc0051 verify final\n" f'sync_dir = "{verify_final_root}"\n' 'bypass_data_preservation = "true"\n'))
        ...
        artifacts = [str(seed_stdout), str(seed_stderr), str(verify_initial_stdout), str(verify_initial_stderr), str(monitor_stdout), str(monitor_stderr), str(verify_final_stdout), str(verify_final_stderr), str(verify_initial_manifest_file), str(verify_final_manifest_file), str(metadata_file)]
        details = {"root_name": root_name, "relative_path": relative_path, "initial_local_hash": initial_local_hash}
        ...
        verify_initial_command = [context.onedrive_bin, "--display-running-config", "--sync", "--download-only", "--verbose", "--resync", "--resync-auth", "--single-directory", root_name, "--syncdir", str(verify_initial_root), "--confdir", str(conf_verify_initial)]
        context.log(f"Executing Test Case {self.case_id} initial verify: {command_to_string(verify_initial_command)}")
        verify_initial_result = self._run_verify_command(context, verify_initial_command, verify_initial_stdout, verify_initial_stderr)
        ...
        if verify_initial_result.returncode != 0 or not verify_initial_file_path.is_file():
            self._write_metadata(metadata_file, details)
            return TestResult.fail_result(self.case_id, self.name, "Initial remote verification failed before monitor mtime-only test", artifacts, details)
        ...
        try:
            ...
            if local_hash_after_touch != initial_local_hash:
                self._write_metadata(metadata_file, details)
                return TestResult.fail_result(self.case_id, self.name, "Local file hash changed after mtime-only touch", artifacts, details)

            required_patterns = [f"[M] Local file changed: {relative_path}", f"Uploading modified file: {relative_path} ... done"]
        ...
        verify_final_command = [context.onedrive_bin, "--display-running-config", "--sync", "--download-only", "--verbose", "--resync", "--resync-auth", "--single-directory", root_name, "--syncdir", str(verify_final_root), "--confdir", str(conf_verify_final)]
        context.log(f"Executing Test Case {self.case_id} final verify: {command_to_string(verify_final_command)}")
        verify_final_result = self._run_verify_command(context, verify_final_command, verify_final_stdout, verify_final_stderr)
        ...
        details["verify_final_hash"] = compute_quickxor_hash_file(verify_final_file_path) if verify_final_file_path.is_file() else ""
        details["verify_final_content"] = verify_final_file_path.read_text(encoding="utf-8") if verify_final_file_path.is_file() else ""
        details["final_verified_mtime"] = int(verify_final_file_path.stat().st_mtime) if verify_final_file_path.is_file() else -1
        self._write_metadata(metadata_file, details)

        if not details.get("mutation_processed", False):
            return TestResult.fail_result(self.case_id, self.name, "Monitor mode did not process the mtime-only local change before shutdown", artifacts, details)
        if verify_final_result.returncode != 0:
            return TestResult.fail_result(self.case_id, self.name, f"Final remote verification failed with status {verify_final_result.returncode}", artifacts, details)
        if not verify_final_file_path.is_file():
            return TestResult.fail_result(self.case_id, self.name, f"Remote verification is missing mtime-only file: {relative_path}", artifacts, details)
        if details["verify_final_hash"] != initial_local_hash or details["verify_final_content"] != initial_content:
            return TestResult.fail_result(self.case_id, self.name, "File content changed during mtime-only monitor scenario", artifacts, details)
        if abs(details["final_verified_mtime"] - details["touched_epoch"]) > 2:
            return TestResult.fail_result(self.case_id, self.name, f"Final remote mtime {details['final_verified_mtime']} did not match touched local timestamp {details['touched_epoch']} within tolerance", artifacts, details)
