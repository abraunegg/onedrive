...
class TestCase0027WhitespaceTrailingDotValidation(E2ETestCase):
    case_id = "0027"
    name = "whitespace and trailing dot validation"
    description = "Validate trailing whitespace and trailing dot names are blocked while valid sibling files still synchronise"
    ...
    def run(self, context: E2EContext) -> TestResult:
        case_work_dir = context.work_root / "tc0027"
        case_log_dir = context.logs_dir / "tc0027"
        state_dir = context.state_dir / "tc0027"
        ...
        root_name = f"ZZ_E2E_TC0027_{context.run_id}_{os.getpid()}"

        valid_files = [
            f"{root_name}/valid_file_1.bin",
            f"{root_name}/middle space valid.txt",
            f"{root_name}/name.with.periods.txt",
        ...
        invalid_files = [
            ...
            f"{root_name}/trailing-space-dir /child.txt",
            f"{root_name}/trailing-dot-dir./child.txt",
        ]
        ...
        for unwanted in invalid_files:
            if unwanted in remote_manifest:
                return TestResult.fail_result(
                    self.case_id,
                    self.name,
                    f"Invalid whitespace or trailing dot name was synchronised remotely: {unwanted}",
                    artifacts,
                    details,
                )
        ...
        for marker in disallowed_remote_failure_markers:
            if marker in combined_output:
                return TestResult.fail_result(
                    self.case_id,
                    self.name,
                    f"Client attempted remote invalid-name operation instead of local skip: {marker}",
                    artifacts,
                    details,
                )
