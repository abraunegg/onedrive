...
class TestCase0009UploadOnlyNoRemoteDelete(E2ETestCase):
    ...
    def run(self, context: E2EContext) -> TestResult:
        ...
        upload_command = [context.onedrive_bin, "--display-running-config", "--sync", "--verbose", "--upload-only", "--no-remote-delete", "--resync", "--resync-auth", "--single-directory", root_name, "--syncdir", str(sync_root), "--confdir", str(upload_conf)]
        ...
        write_text_file(metadata_file, "\n".join([f"root_name={root_name}", f"seed_returncode={seed_result.returncode}", f"upload_returncode={upload_result.returncode}", f"verify_returncode={verify_result.returncode}", f"verify_scope={root_name}"]) + "\n")
        ...
        details = {"seed_returncode": seed_result.returncode, "upload_returncode": upload_result.returncode, "verify_returncode": verify_result.returncode, "root_name": root_name, "verify_scope": root_name}
