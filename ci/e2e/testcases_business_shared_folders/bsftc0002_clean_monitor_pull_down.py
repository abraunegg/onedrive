...
class BusinessSharedFolderTestCase0002CleanMonitorPullDown(E2ETestCase):
    case_id = "bsftc0002"
    name = "business shared folders clean monitor pull down"
    description = "Validate that --monitor --verbose pulls down the preserved Business Account shared-folder topology"
    ...
    def run(self, context: E2EContext) -> TestResult:
        ...
        validation = validate_required_manifest(
            sync_root=sync_root,
        ...
        if not initial_sync_complete:
            return self.fail_result(
                self.case_id,
                self.name,
                "Monitor mode did not complete the initial Business shared-folder sync within the expected time",
                artifacts,
                details,
            )
